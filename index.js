...
/**
 * Extends app.io objects, adding middleware functionality and event and
 * namespace properties, which stores event name and namespace (before ':' part)
 * respectively.
...
module.exports = function(app) {
   ...
   * @param {Function|Object} next Signal handler or request object (according
   * to options.trigger), as request object used for internal express.io
   * purposes
   * @param {Object} [options] Additional parameters, options.trigger makes
   * sense and used for internal express.io purposes
  ...
  app.io.route = function(route, next, options) {
    ...
    if (_.isFunction(next)) {
      ...
      for (var key in next) {
        ...
        _results.push(registerRoute(this.router, this.middleware, "" + route +
            ":" + key, next[key]));
      }
      return _results;
    }
   ...
   * @param {RegExp|Function} regexp RegExp, which specifies events, for which
...
function registerRoute(router, middlewares, route, func) {
  ...
  if (previousFunc) { // if another handler exists, adding new in chain
    ...
    // compile all middlewares in chain
      if (middleware[0].test(route)) {
        if (previousFunc) {
          previousFunc = router[route] =
              createRouteFunction(previousFunc, middleware[1]);
        } else {
          previousFunc = router[route] =
              createRouteFunction(middleware[1], route);
        }
      }
    });
 ...
 * @param {Function} previousFunc Previous function in chain or new function in
 * chain, according to second argument
